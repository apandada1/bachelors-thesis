...
We can now put $d = 2$ and use the expression of $\dot{\vec{r}}$ in Eq. \eqref{Eq:SC_EOM_r_dot_decoupled} to further simplify Eq. \eqref{Eq:electric-current} and Eq. \eqref{Eq:heat-current}, and obtain \footnote{This is not the complete story. There are other terms because the electron is not completely localized in the center of the wavepacket, and play an important role. More about that in section \ref{chap:OrbMag}},
...
The zeroeth order term in magnetic field is 0, as $\int \frac{d\vec{k}}{(2\pi)^d} f_{\vec{k}} \frac{\partial \varepsilon_0} {\vec{k}} = 0$.
...
\begin{aligned}
	...
	= & (-1)^n \int_{\text{BZ}} \frac{d\vec{k}}{(2\pi)^d} (\frac{\partial}{\partial \vec{k}}\frac{\partial^{n-1} f}{\partial \varepsilon_{0}^n}) \frac{(\vec{m} \cdot \vec{B})^n}{n !} + \frac{\partial^{n-1} f}{\partial \varepsilon_{0}^{n-1}} \frac{(\vec{m} \cdot \vec{B})^{n-1}}{(n-1) !} \frac{\partial }{\partial \vec{k}}(\vec{m}\cdot\vec{B}) \\
...
\end{aligned},
$$
because the integral of the gradient of a periodic scalar, over the Brillouin Zone is zero.
...
\begin{equation}\label{Eq:Onsager-relation}
	\stackrel{\leftrightarrow}{L}_{12} = T \stackrel{\leftrightarrow}{L}_{21}.
\end{equation}
Relations like this are known as \textit{Onsager relation}s. In this particular case, the Onsager relation is equivalent to the relation between the Seebeck and Peltier coefficients (See chapter \ref{chap:Seebeck-Peltier}).
...
Special case (Regular Ohmic conduction): When $\vec{B} = 0$, we get $ g = \frac{\partial f} {\partial \varepsilon}
...
\textit{Onsager Relation}: In Eq. \eqref{Eq:g_non_zero_chem_temp_grad}, $\nabla T$ always comes with a factor of $\frac{\varepsilon - \mu}{T}$, and to find the heat current, we need to multiply with an additional factor of $({\varepsilon - \mu})$ (see \ref{Eq:heat-current}). It follows that the contribution to $\stackrel{\leftrightarrow}{L}_{12}$ and $\stackrel{\leftrightarrow}{L}_{21}$ are the same upto a factor of $\frac{1}{T}$, and the Onsager relation (see Eq. \ref{Eq:Onsager-relation}) holds for these contributions.
...
\begin{equation}\label{Eq:OrbitalMagneticMomentExpression}
	\vec{m}_{\vec{k}} = -\frac{e}{2 m} \bra{\psi_{\vec{k}}}(\hat{\vec{r}} - \vec{r}_0)\times\hat{\vec{p}}\ket{\psi_{\vec{k}}}
\end{equation}
This is just a generalization of the formula $\vec{m} = (-e)g \frac{\vec{L}}{2 m}$, with the Lande-g factor set to 1, because this is due to the orbital motion of the electron, not due to its spin.
...
Not necessarily. Consider a free particle moving in a ring. The eigenfunctions would have zero Berry curvature, but there is a net magnetic moment.
...
\end{aligned}
$$
This is nothing but the $\mu$-th component of $\nabla \times \int \frac{d\vec{k}}{(2\pi)^d} (f+g) (1+\frac{e}{\hbar} \vec{B}\cdot\vec{\Omega}) \vec{m}_{\vec{k}}$.

When the electric field, chemical potential gradient and the temperature gradients are constant, we can drop the term $\nabla \times \int d\vec{k} g (1+\frac{e}{\hbar} \vec{B}\cdot\vec{\Omega}) \vec{m}_{\vec{k}}$, because $g$ is already a linear function of $\vec{E}$, $\nabla \mu$ and $\nabla T$.
...
However, the net magnetization density is slightly different from the sum of the individual orbital magnetic moments.
...
$$
\begin{aligned}
\vec{M} &= -\frac{\partial G}{\partial \vec{B}} |_{\vec{B} = 0} \\
...
\begin{equation}~\label{Eq:transport-electric}
	\begin{aligned}
	{{\vec{j}}^e}_{\text{tr}} &= \expval{\hat{\vec{j}}_e} - {\vec{j}^e}_{M} \\
	&= \int \frac{d\vec{k}}{(2\pi)^d} (-e){g}_{\vec{k}} \frac{1}{\hbar} \frac{\partial \varepsilon_0({{\vec{k}}})}{\partial \vec{k}} - {f}_{\vec{k}} \frac{e^2}{\hbar} \left(\left[\vec{E} + \frac{\nabla \mu}{e} + \frac{(\varepsilon_0(\vec{k}) - \mu)}{T} \nabla T \right] \times \vec{\Omega}(\vec{k})\right) \\
	&- \frac{\nabla T}{T}  \times \int  \frac{d\vec{k}}{(2\pi)^d} \frac{e}{\hbar} \vec{\Omega}(\vec{k}) k_B T \log(1 + e^{-\beta(\varepsilon_0(\vec{k}) - \mu)})
	\end{aligned}
\end{equation}
We now see that the \textit{Einstein relation} is valid because we can exchange $\vec{E} \leftrightarrow \frac{\nabla \mu}{e}$. To show that the Onsager relation is valid, we need to find the modified expression of the heat current, taking into account of the magnatization density.

\section{Modified Expression for Heat Current Density}
We can use a similar approach for the heat current density, and it has been shown that \cite{PhysRevLett.97.026603},
\begin{equation} \label{Eq:heat-current-simplified-including-magnetization}
	\expval{\hat{\vec{j}}_Q} = \int \frac{d\vec{k}}{(2\pi)^d} (\varepsilon_{_{\vec{k}}} - \mu) ({f}_{\vec{k}} + {g}_{\vec{k}}) \left[\frac{1}{\hbar} \frac{\partial \varepsilon_{_{\vec{k}}}}{\partial \vec{k}} + \frac{e}{\hbar} (\vec{E}\times\vec{\Omega}(\vec{k}))\right]
	- \vec{E} \times \int \frac{d\vec{k}}{(2\pi)^d}\left(1+\frac{e}{\hbar} \vec{B}\cdot\vec{\Omega}\right)
	 (f + g) \vec{m}_{\vec{k}}
\end{equation}
