...
The differential equations of motion for $\dot{\vec{r}}$ and $\dot{\vec{k}}$ (Eq. \eqref{Eq:evolution-of-R} and Eq. \eqref{Eq:evolution-of-K}) can be easily decoupled (See Appendix \ref{app:decoupling-of-eom}), and the resulting decoupled equations are,
...
\begin{equation}~\label{Eq:TrasportCoeffs}
	...
	\end{aligned}
\end{equation}
Here $\stackrel{\leftrightarrow}{L}_{i j}$ ($i,j \in {1,2}$) are tensors, they act on a vector to give a vector. $\stackrel{\leftrightarrow}{L}_{1 1}$ is the electric conductivity, $\stackrel{\leftrightarrow}{L}_{2 2}$ is the thermal conductivity\footnote{Sometimes, thermal conductivity is denoted as heat current per unit temperature gradient, when the electric current is zero. In that case, it can be shown that (see Eq. 13.56 of \cite{book:AshcroftMermin76}) the heat current is $\stackrel{\leftrightarrow}{L}_{22} - \stackrel{\leftrightarrow}{L}_{21} (\stackrel{\leftrightarrow}{L}_{11})^{-1} \stackrel{\leftrightarrow}{L}_{12}$.}, and $\stackrel{\leftrightarrow}{L}_{1 2}$, $\stackrel{\leftrightarrow}{L}_{2 1}$ are related to the Seebeck and Peltier coefficients.
...
\begin{equation}~\label{Eq:transport-electric}
	...
	\end{aligned}}
\end{equation}
We now see that the \textit{Einstein relation} is valid because we can exchange $\vec{E} \leftrightarrow \frac{\nabla \mu}{e}$ (the validity of the Einstein relation in the non-equilibrium distribution $g_{\vec{k}}$ has already been shown). To show that the Onsager relation is valid, we need to find the modified expression of the heat current, taking into account of the magnetization density.
...
$$
	...
	Let $\vec{k} + \Delta \vec{k} = \vec{k}'$. Note that upto linear order in $\Delta t$, $\Delta \vec{k} = \Delta \vec{k}'$. Then, after changing variables in the integral,
	$$
...
\end{aligned}
$$
Therefore, $\frac{d}{dt} \log(\Delta V) = -\frac{d}{dt} \log(1 + \frac{e}{\hbar} \vec{B}\cdot\vec{\Omega})$, i.e. $\frac{d}{dt} \log[V(1 + \frac{e}{\hbar} \vec{B}\cdot\vec{\Omega})] = 0$.
...
Recall that the effective Hamiltonian acting on $u_{n, \vec{k}}$ is, $H(\vec{k})  = \left[\frac{\hbar^2}{2m}(\vec{k} - i \nabla)^2 + V(\vec{r}) \right] = e^{-i \vec{k}\cdot\vec{r}} \hat{H} e^{i \vec{k}\cdot\vec{r}}$.
Let us define $\vec{P}(\vec{k})= \frac{m}{\hbar} \frac{\partial H(\vec{k})}{\partial \vec{k}} = \hbar(\vec{k} - i\nabla)$.

\subsection{Calculation of $\bra{u_{\vec{k}}} \vec{P}(\vec{k}) \ket{u_{\vec{k}}}$}
The idea of this calculation is based on \cite{chang1996}.

$$
\begin{aligned}
	\expval{\vec{P}(\vec{k})} = \bra{u_{\vec{k}}} \vec{P}(\vec{k}) \ket{u_{\vec{k}}} &= \frac{m}{\hbar} \bra{u_{\vec{k}}} \frac{\partial H(\vec{k})}{\partial \vec{k}} \ket{u_{\vec{k}}} \\
...
\end{aligned},
$$
which is just the (bare) electron mass times its velocity. Note that the Berry curvature term does not appear here because we took a single Bloch eigenstate, not an wavepacket.

\subsection{Calculation of $\bra{\frac{\partial}{\partial \vec{k}} u_{\vec{k}}} \vec{P}(\vec{k}) \ket{u_{\vec{k}}}$}
This calculation is based on \cite{chang1996}.

$$
\begin{aligned}
	\bra{\frac{\partial}{\partial k_\mu} u_{\vec{k}}} P_\nu (\vec{k}) \ket{u_{\vec{k}}} &= \frac{m}{\hbar} \bra{\frac{\partial}{\partial k_\mu} u_{\vec{k}}} \frac{\partial H(\vec{k})}{\partial k_\nu} \ket{u_{\vec{k}}} \\
...
$$
\begin{aligned}
	\frac{1}{2}\bra{\psi_{\vec{k},\vec{r}_1}} \frac{1}{2}\{-e\frac{\hat{p}_k}{m}, (\vec{\hat{r}} - \vec{r}_1)_j\} \ket{\psi_{\vec{k},\vec{r}_1}} \epsilon_{ijk} & = -\frac{e}{2m} \epsilon_{ijk} \bra{\psi_{\vec{k},\vec{r}_1}}(\vec{\hat{r}} - \vec{r}_1)_j \hat{p}_k  \ket{\psi_{\vec{k},\vec{r}_1}} \\
